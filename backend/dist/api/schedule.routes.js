...
const queue_2 = require("../queue/queue");
...
router.post('/schedule', async (req, res) => {
    try {
        const { recipients, subject, body, sender = 'outbox@reachinbox.ai', sendAt, delaySeconds = 2, hourlyLimit = 100, } = req.body;
        ...
        for (let i = 0; i < uniqueRecipients.length; i++) {
            ...
            const emailRecord = await client_1.prisma.scheduledEmail.create({
                data: {
                    recipient,
                    subject,
                    body,
            ...
            const enqueued = await (0, queue_1.enqueueEmailJob)(emailRecord.id, staggeredSendAt);
            if (enqueued)
                enqueuedCount++;
...
router.get('/scheduled', async (req, res) => {
    try {
        ...
        const [emails, total] = await Promise.all([
            client_1.prisma.scheduledEmail.findMany({
                where: whereCondition,
                orderBy: { sendAt: 'asc' },
                skip,
                take: limit,
            }),
            client_1.prisma.scheduledEmail.count({ where: whereCondition }),
        ]);
        res.json({
            success: true,
            emails,
...
router.get('/stats', async (req, res) => {
    try {
        ...
        const [scheduledCount, sentCount, failedCount, rateInfo] = await Promise.all([
            client_1.prisma.scheduledEmail.count({ where: { status: { in: ['SCHEDULED', 'SENDING'] } } }),
            client_1.prisma.scheduledEmail.count({ where: { status: 'SENT' } }),
            client_1.prisma.scheduledEmail.count({ where: { status: 'FAILED' } }),
            (0, hourlyLimiter_1.getCurrentHourlyUsage)(queue_2.redisConnection, defaultSender),
        ]);
        res.json({
            success: true,
            stats: {
...
router.delete('/:id', async (req, res) => {
    try {
        const { id } = req.params;
        const email = await client_1.prisma.scheduledEmail.findUnique({ where: { id } });
        if (!email)
        ...
        await client_1.prisma.scheduledEmail.delete({ where: { id } });
