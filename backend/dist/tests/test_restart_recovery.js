...
async function testRestartRecovery() {
    ...
    const pendingEmails = await client_1.prisma.scheduledEmail.findMany({
    ...
    while (Date.now() - startWait < 15000) {
        updatedEmail = await client_1.prisma.scheduledEmail.findUnique({ where: { id: emailRecord.id } });
    ...
    const checkAgain = await client_1.prisma.scheduledEmail.findUnique({ where: { id: emailRecord.id } });
