...
export function getHourWindowKey(sender: string, date: Date = new Date()): { key: string; msUntilNextHour: number } {
...
export async function getCurrentHourlyUsage(
  redis: Redis | null,
  sender: string
): Promise<{ count: number; hourKey: string }> {
